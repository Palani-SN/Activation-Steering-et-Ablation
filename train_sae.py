...
def train_sae_from_payload(payload_path, epochs=100):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            print(
                f"  [{bar}] Epoch {epoch+1:3d}/{epochs} | MSE: {avg_mse:.6f} | {pct:5.1f}%")
            mse.append((int(epoch+1), float(avg_mse)))
    ...
    return "sae/universal_sae.pth", mse
...
if __name__ == "__main__":
    ...
    file, data = train_sae_from_payload("temp/harvested_data.pt", epochs=100)
    ...
    plt.title(file)
    plt.grid(True)
    plt.savefig("images/sae_training.png")
    plt.close()
